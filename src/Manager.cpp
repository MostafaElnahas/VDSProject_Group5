...
bool ClassProject::Manager::isVariable(BDD_ID x) {
    ...
    if ((unique_table[x].Low == 0) && (unique_table[x].High == 1)) {
        return true;
    }
...
/*    for (const auto &unique_table: unique_table) {
    ...
    BDD_ID high = unique_table[f].High;
    BDD_ID low = unique_table[f].Low;
...
std::string ClassProject::Manager::getTopVarName(const BDD_ID &root) {
    return unique_table[unique_table[root].TopVar].label;
/*    BDD_ID Y;
    Y = topVar(root);
    for (const auto &unique_table: unique_table) {
